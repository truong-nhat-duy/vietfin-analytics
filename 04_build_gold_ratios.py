...
# 1. Khởi tạo kết nối MotherDuck từ .env
load_dotenv()
token = os.getenv("MOTHERDUCK_TOKEN")

if not token:
    raise ValueError("LỖI: Không tìm thấy MOTHERDUCK_TOKEN trong file .env!")

con = duckdb.connect(f"md:vietfin_db?token={token}")
print("---> Kết nối MotherDuck thành công. Bắt đầu xử lý Tầng Gold...")

# 2. SQL Pivot dữ liệu & Tính toán các chỉ số tài chính
gold_sql = """
...
# 3. Thực thi tính toán trên MotherDuck Cloud
con.execute(gold_sql)

# 4. Kiểm tra tổng số bản ghi
...
# 5. Lưu bản sao Parquet về Local
